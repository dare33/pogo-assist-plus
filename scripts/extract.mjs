...
//
// Frames are decoded with ffmpeg: FFMPEG env var, then `ffmpeg` on the PATH, then the binary
// that the imageio-ffmpeg Python package installs (development convenience only; the web page
// decodes in the browser and needs neither). Pass a directory of PNGs to skip decoding.
...
export function decodeFrames(video, dir, fps) {
  const ffmpeg = findFfmpeg();
  if (!ffmpeg) throw new Error('ffmpeg not found: set FFMPEG, put ffmpeg on the PATH, or `pip install imageio-ffmpeg`');
...
const ocr = await createOcr(createWorker, { langPath: resolve('data/tessdata'), cachePath: resolve('data/tessdata'), gzip: false });
const { rows, review, readings } = await extract(pngFrames(framesDir, fps), {
  ocr, gm, total,
...
writeFileSync(outReview, JSON.stringify({ input, fps, frames: total, rows: rows.length, flagged: review.length, review, readings: readings.map((r) => ({ frame: r.frame, cp: r.cp, cpText: r.cpText, name: r.name, nameText: r.nameText, nameConfidence: Math.round(r.nameConfidence), hp: r.hp, ivs: r.ivs, ivConfidence: r.ivConfidence && Number(r.ivConfidence.toFixed(2)), fills: r.fills?.map((f) => Number((f * 15).toFixed(2))), sharpness: Math.round(r.sharpness), flags: r.flags })) }, null, 2));

if (!quiet) {
  console.error(`\n${rows.length} Pokémon from ${total} frames in ${((Date.now() - t0) / 1000).toFixed(1)} s; ${review.length} flagged for review`);
